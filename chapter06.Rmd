...
--- type:NormalExercise lang:r xp:100 skills:1 key:826d4acb52
## Fix the incorrect code

Recall the `bechdel` data frame you have worked with previously.  Your friend is trying to produce the side-by-side barplot shown, but is having a really tough time.  Fix their code below to produce the plot.  Note the addition of `labs` was made to change the labels for each of the axes and the title.

*** =instructions

*** =hint
- Use `?labs` to see how to change the labels of plots.
...
library(fivethirtyeight)
library(ggplot2)
ggplot(data = flying, mapping = aes(x = age, fill = recline_rude)) +
    geom_bar(position = "fill", color = "white")
```
