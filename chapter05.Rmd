...
*** =instructions
- Use `position = "fill"` to create a barplot in the spirit of **The Bechdel Test Over Time** image in the article ["The Dollar-And-Cents Case Against Hollywood’s Exclusion of Women"](https://fivethirtyeight.com/features/the-dollar-and-cents-case-against-hollywoods-exclusion-of-women/).
- THINK ABOUT IT:  What is clearer in observing this plot than the plots created in the previous three exercises?
...
We could continue to tweak the plot to make it look more and more like the actual published plot using nothing but `ggplot` code.  This is one of the great powers of R:  its customizability and portability make creating professional-style graphics interactive and fun and REPRODUCIBLE!

*** =instructions
- After writing the code to produce the plot shown, add in the code given to color the bars appropriately.  (Remember to use the Console as your sandbox for testing.)
